 ...
 * @Project: MrData
 * @Filename: resourcesmined.js
 * @Last modified time: 2019-05-08T16:43:42+01:00
 */
...
module.exports = {
    name: 'resourcesmined',
    description: 'See the top 10 resources mined scores, or set your own resources mined score.',
    aliases: ['rm'],
    args: false,
    usage: '<number>',
    cooldown: 3,
    ...
    execute(message, args) {
        ...
        logger.debug(`Executing resourcesmined, args: ${JSON.stringify(args)}`);
        switch (args.length) {
            case 0:
                /*
 				 */
                ...
                db.scores.findByUserAndGuild(message.author.id, message.guild.id)
                    .then(author_score => {
                        db.scores.findByGuild(message.guild.id, 'resources_mined DESC LIMIT 10')
                            .then(top10 => {
                                ...
                                if (top10 == null || top10.length == 0) {
                                    ...
                                    const embed = new Discord.RichEmbed()
                                        .setTitle("resources mined Leaderboard")
                                        .setAuthor(message.client.user.username, message.client.user.avatarURL)
                                        .setDescription("Our top 10 Resources Mined scores!")
                                        .setColor(config.resourcesminedColor);
                                    var c = 1;
                                    for (const data of top10) {
                                        ...
                                        embed.addField(`${c}. ${displayName}`, `${library.Format.numberWithCommas(data.resources_mined)}`);
                                        c++;
                                    }
                                    if (author_score == null || author_score.length == 0) {
                                        embed.addField(`*You have not yet set any scores!*`);
                                    } else {
                                        logger.debug(`${message.author} score ${author_score.resources_mined}`);
                                        embed.addField(`*Your personal resources mined score is*`, `*${library.Format.numberWithCommas(author_score.resources_mined)}*`);
                                    }
                ...
                if (isNaN(args[0])) {
                    ...
                    if (member) {
                        ...
                        db.scores.findByUserAndGuild(member.id, message.guild.id)
                            .then(score => {
                                ...
                                if (score != null) {
                                    ...
                                    desc = `${member.displayName} resources mined is ${library.Format.numberWithCommas(score.resources_raided)}`
                                } // endif
                                message.channel.send({
                                    embed: {
                                        color: config.resourcesminedColor,
                        ...
                        message.channel.send({
                            embed: {
                                color: config.resourcesminedColor,
                                description: `${message.author}, please use \`!resourcesmined abc\`, where abc is a number or an actual person!}`
                            }
                        });
                        return;
                    } // endif member
                    ...
                    new_score.resources_mined = args[0];
                    new_score.user_discord_id = message.author.id;
                    new_score.success_message = `Thank you, ${message.author}, your resources mined is set to ${library.Format.numberWithCommas(new_score.resources_mined)}`;
                }
                break;
            case 2:
                {
                    ...
                    if (member && !isNaN(args[1])) {
                        // The command seems to be of the form !resourcesmined @name number
                        ...
                        if (!message.member.roles.has(allowedRole.id)) {
                            ...
                            new_score.resources_mined = args[1];
                            new_score.user_discord_id = member.id;
                            new_score.success_message = `Thank you, ${message.author}, ${member.displayName} resources mined is set to ${library.Format.numberWithCommas(new_score.resources_mined)}`
                        }
                    }
                    break;
                }
