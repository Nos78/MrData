 ...
 * @Last Modified time: 2020-09-28 22:48:43
 */
...
module.exports = {
    ...
    if(maxRankCount < 1) {
      ...
      return;
		...
		logger.debug(`Executing allianceshelpsent, args: ${JSON.stringify(args)}`);
		switch(args.length) {
            case 0:
                /*
 				 */
                ...
                db.scores.findByUserAndGuild(message.author.id, message.guild.id)
                    .then(author_score => {
                        db.scores.findByGuild(message.guild.id, `alliances_help DESC LIMIT ${maxRankCount}`)
                            .then(top10 => {
                                ...
                                if (top10 == null || top10.length == 0) {
                                    ...
                                    for (const data of top10) {
                                        ...
                                        embed.addField(`${c}. ${displayName}`, `${library.Format.numberWithCommas(data.alliances_help)}`);
                                        c++;
                                    }
                                    if (author_score == null || author_score.length == 0) {
                                        embed.addField(`*You have not yet set any scores!*`);
                                    } else {
                                        logger.debug(`${message.author} score ${author_score.alliances_help}`);
                                        embed.addField(`*Your personal alliances help sent is*`, `*${library.Format.numberWithCommas(author_score.alliances_help)}*`);
                                    }
              ...
              .then (score => {
                let desc = `Unable to find ${member.displayName} in my database.  They need to log their scores for you to view them!`;
								if(score!=null) {
									...
									desc = `${member.displayName} alliances help sent is ${library.Format.numberWithCommas(score.alliances_help)}`
								} // endif
                        ...
                        message.channel.send({embed: {
                            color: config.alliancesHelpSentColor,
                            description: `${message.author}, please use \`!allianceshelpsent abc\`, where abc is a number or an actual person!}`
                        }});
						return;
					} // endif member
				} else {
					...
					new_score.alliances_help = args[0];
					new_score.user_discord_id = message.author.id;
					new_score.success_message = `Thank you, ${message.author}, your alliances help sent is set to ${library.Format.numberWithCommas(new_score.alliances_help)}`;
				}
				break;
            case 2:
                {
                    ...
                    if (member && !isNaN(args[1])) {
                        // The command seems to be of the form !allianceshelpsent @name number
                        ...
                        if (!message.member.roles.has(allowedRole.id)) {
                            ...
                            new_score.alliances_help = args[1];
                            new_score.user_discord_id = member.id;
                            new_score.success_message = `Thank you, ${message.author}, ${member.displayName} alliances help sent is set to ${library.Format.numberWithCommas(new_score.alliances_help)}`
                        }
                    }
                    break;
                }
        ...
        db.scores.findByUserAndGuild(new_score.user_discord_id, new_score.guild_discord_id)
            .then(score => {
                if (score == null) {
                    ...
                    score.alliances_help = new_score.alliances_help
