 ...
 * @Last modified time: 2019-05-08T15:26:19+01:00
 */
...
module.exports = {
    name: 'powerdestroyed',
    description: 'See the top 10 power destroyed scores, or set your own power destroyed score.',
    aliases: ['pd'],
    args: false,
    usage: '<number>',
    cooldown: 3,
    ...
    execute(message, args) {
        ...
        logger.debug(`Executing powerdestroyed, args: ${JSON.stringify(args)}`);
        switch (args.length) {
            case 0:
                /*
 				 */
                            ...
                            .then(top10 => {
                                ...
                                if (top10 == null || top10.length == 0) {
                                    ...
                                    const embed = new Discord.RichEmbed()
                                        ...
                                        .setDescription("Our top 10 power destroyed scores!")
                                    ...
                                    for (const data of top10) {
                                        ...
                                        embed.addField(`${c}. ${displayName}`, `${library.Format.numberWithCommas(data.power_destroyed)}`);
                                        c++;
                                    }
                                    if (author_score == null || author_score.length == 0) {
                                        embed.addField(`*You have not yet set any scores!*`);
                                    } else {
                                        logger.debug(`${message.author} score ${author_score.power_destroyed}`);
                                        embed.addField(`*Your personal power destroyed score is*`, `*${library.Format.numberWithCommas(author_score.power_destroyed)}*`);
                                    }
            ...
            case 1:
                ...
                if (isNaN(args[0])) {
                        ...
                        db.scores.findByUserAndGuild(member.id, message.guild.id)
                            .then(score => {
                                ...
                                if (score != null) {
                                    ...
                                    desc = `${member.displayName} power destroyed is ${library.Format.numberWithCommas(score.resources_raided)}`
                                } // endif
                                message.channel.send({
                                    embed: {
                                        color: config.powerDestroyedColor,
                    ...
                    new_score.power_destroyed = args[0];
                    new_score.user_discord_id = message.author.id;
                    new_score.success_message = `Thank you, ${message.author}, your power destroyed is set to ${library.Format.numberWithCommas(new_score.power_destroyed)}`;
                }
                break;
            case 2:
                {
                    ...
                    if (member && !isNaN(args[1])) {
                        ...
                        if (!message.member.roles.has(allowedRole.id)) {
                            ...
                            new_score.power_destroyed = args[1];
                            new_score.user_discord_id = member.id;
                            new_score.success_message = `Thank you, ${message.author}, ${member.displayName} power destroyed is set to ${library.Format.numberWithCommas(new_score.power_destroyed)}`
                        }
                    }
                    break;
                }
