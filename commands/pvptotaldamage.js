 ...
 * @Last modified time: 2019-05-08T16:43:37+01:00
 */
...
module.exports = {
    name: 'pvptotaldamage',
    description: 'See the top 10 pvp total damage scores, or set your own pvp total damage score.',
    aliases: ['ptd'],
    args: false,
    usage: '<number>',
    cooldown: 3,
    ...
    execute(message, args) {
        ...
        logger.debug(`Executing pvptotaldamage, args: ${JSON.stringify(args)}`);
        switch (args.length) {
            case 0:
                /*
 				 */
                ...
                db.scores.findByUserAndGuild(message.author.id, message.guild.id)
                    .then(author_score => {
                        db.scores.findByGuild(message.guild.id, 'pvp_total_damage DESC LIMIT 10')
                            .then(top10 => {
                                ...
                                if (top10 == null || top10.length == 0) {
                                    ...
                                    message.channel.send({
                                        embed: {
                                            color: config.pvptotaldamageColor,
                                    ...
                                    const embed = new Discord.RichEmbed()
                                        .setTitle("pvp total damage Leaderboard")
                                        .setAuthor(message.client.user.username, message.client.user.avatarURL)
                                        .setDescription("Our top 10 PvP Total Damage scores!")
                                        .setColor(config.pvptotaldamageColor);
                                    var c = 1;
                                    for (const data of top10) {
                                        ...
                                        embed.addField(`${c}. ${displayName}`, `${library.Format.numberWithCommas(data.pvp_total_damage)}`);
                                        c++;
                                    }
                                    if (author_score == null || author_score.length == 0) {
                                        embed.addField(`*You have not yet set any scores!*`);
                                    } else {
                                        logger.debug(`${message.author} score ${author_score.pvp_total_damage}`);
                                        embed.addField(`*Your personal pvp total damage score is*`, `*${library.Format.numberWithCommas(author_score.pvp_total_damage)}*`);
                                    }
                ...
                if (isNaN(args[0])) {
                    ...
                    if (member) {
                        ...
                        db.scores.findByUserAndGuild(member.id, message.guild.id)
                            .then(score => {
                                ...
                                if (score != null) {
                                    ...
                                    desc = `${member.displayName} pvp total damage is ${library.Format.numberWithCommas(score.resources_raided)}`
                                } // endif
                                message.channel.send({
                                    embed: {
                                        color: config.pvptotaldamageColor,
                        ...
                        message.channel.send({
                            embed: {
                                color: config.pvptotaldamageColor,
                                description: `${message.author}, please use \`!pvptotaldamage abc\`, where abc is a number or an actual person!}`
                            }
                        });
                        return;
                    } // endif member
                    ...
                    new_score.pvp_total_damage = args[0];
                    new_score.user_discord_id = message.author.id;
                    new_score.success_message = `Thank you, ${message.author}, your pvp total damage is set to ${library.Format.numberWithCommas(new_score.pvp_total_damage)}`;
                }
                break;
            case 2:
                {
                    ...
                    if (member && !isNaN(args[1])) {
                        // The command seems to be of the form !pvptotaldamage @name number
                        ...
                        if (!message.member.roles.has(allowedRole.id)) {
                            ...
                            new_score.pvp_total_damage = args[1];
                            new_score.user_discord_id = member.id;
                            new_score.success_message = `Thank you, ${message.author}, ${member.displayName} pvp total damage is set to ${library.Format.numberWithCommas(new_score.pvp_total_damage)}`
                        }
                    }
                    break;
                }
        ...
        db.scores.findByUserAndGuild(new_score.user_discord_id, new_score.guild_discord_id)
            .then(score => {
                if (score == null) {
                    ...
                    score.pvp_total_damage = new_score.pvp_total_damage
