 ...
 * @Project: MrData
 * @Filename: hostileshipsdestroyed.js
 * @Last modified time: 2020-03-29T19:14:42+01:00
 */
...
module.exports = {
    name: 'hostileshipsdestroyed',
    description: 'See the top 10 hostile ships destroyed scores, or set your own hostile ships destroyed score. **Custom top X** - *Use !hsd -count X*, where X is a number between 1 and 25, to show the top X scores!',
    aliases: ['hsd'],
    args: false,
    usage: '<number>',
    cooldown: 3,
    ...
    execute(message, args) {
        ...
        if(maxRankCount < 1) {
          ...
          message.channel.send({
              embed: {
                  color: config.hostileshipsdestroyedColor,
        ...
        logger.debug(`Executing hostileshipsdestroyed, args: ${JSON.stringify(args)}`);
        switch (args.length) {
            case 0:
                /*
 				 */
                ...
                db.scores.findByUserAndGuild(message.author.id, message.guild.id)
                    .then(author_score => {
                        db.scores.findByGuild(message.guild.id, `hostiles_destroyed DESC LIMIT ${maxRankCount}`)
                            .then(top10 => {
                                ...
                                if (top10 == null || top10.length == 0) {
                                    ...
                                    const embed = new Discord.RichEmbed()
                                        .setTitle("hostile ships destroyed Leaderboard")
                                        .setAuthor(message.client.user.username, message.client.user.avatarURL)
                                        .setDescription(`Our top ${maxRankCount} Hostile Ships Destroyed scores!`)
                                        .setColor(config.hostileshipsdestroyedColor);
                                    var c = 1;
                                    for (const data of top10) {
                                        ...
                                        embed.addField(`${c}. ${displayName}`, `${library.Format.numberWithCommas(data.hostiles_destroyed)}`);
                                        c++;
                                    }
                                    if (author_score == null || author_score.length == 0) {
                                        embed.addField(`*You have not yet set any scores!*`);
                                    } else {
                                        logger.debug(`${message.author} score ${author_score.hostiles_destroyed}`);
                                        embed.addField(`*Your personal hostile ships destroyed score is*`, `*${library.Format.numberWithCommas(author_score.hostiles_destroyed)}*`);
                                    }
                ...
                if (isNaN(args[0])) {
                    ...
                    if (member) {
                        ...
                        db.scores.findByUserAndGuild(member.id, message.guild.id)
                            .then(score => {
                                ...
                                if (score != null) {
                                    ...
                                    desc = `${member.displayName} hostile ships destroyed is ${library.Format.numberWithCommas(score.resources_raided)}`
                                } // endif
                                message.channel.send({
                                    embed: {
                                        color: config.hostileshipsdestroyedColor,
                        ...
                        message.channel.send({
                            embed: {
                                color: config.hostileshipsdestroyedColor,
                                description: `${message.author}, please use \`!hostileshipsdestroyed abc\`, where abc is a number or an actual person!}`
                            }
                        });
                        return;
                    } // endif member
                    ...
                    new_score.hostiles_destroyed = args[0];
                    new_score.user_discord_id = message.author.id;
                    new_score.success_message = `Thank you, ${message.author}, your hostile ships destroyed is set to ${library.Format.numberWithCommas(new_score.hostiles_destroyed)}`;
                }
                break;
            case 2:
                {
                    ...
                    if (member && !isNaN(args[1])) {
                        // The command seems to be of the form !hostileshipsdestroyed @name number
                        ...
                        if (!message.member.roles.has(allowedRole.id)) {
                            ...
                            new_score.hostiles_destroyed = args[1];
                            new_score.user_discord_id = member.id;
                            new_score.success_message = `Thank you, ${message.author}, ${member.displayName} hostile ships destroyed is set to ${library.Format.numberWithCommas(new_score.hostiles_destroyed)}`
                        }
                    }
                    break;
                }
        ...
        db.scores.findByUserAndGuild(new_score.user_discord_id, new_score.guild_discord_id)
            .then(score => {
                if (score == null) {
                    ...
                    score.hostiles_destroyed = new_score.hostiles_destroyed
